...
validate_mac_address() {
    local mac=$1
    local colon_regex='^([0-9a-fA-F]{2}:){5}[0-9a-fA-F]{2}$'
    local dash_regex='^([0-9a-fA-F]{2}-){5}[0-9a-fA-F]{2}$'
    
    if [[ $mac =~ $colon_regex ]] || [[ $mac =~ $dash_regex ]]; then
...
# Function to normalize MAC address (convert to colon format)
normalize_mac() {
    local mac=$1
    # Convert dashes to colons and ensure lowercase
    echo "$mac" | tr '-' ':' | tr '[:upper:]' '[:lower:]'
}

# Function to find interface by MAC address
find_interface_by_mac() {
    local target_mac=$1
    local normalized_target_mac
    local interface
    ...
    while read -r interface mac; do
        if [ "$mac" = "$normalized_target_mac" ]; then
            echo "$interface"
            return 0
        fi
    done < <(ip link show | awk '/^[0-9]+:/ {gsub(/:/, "", $2); iface=$2} /link\/ether/ {print iface, $2}')
    
    return 1  # Interface not found
}

# Function to validate the network adapter (or MAC address)
validate_adapter() {
    local adapter=$1
    local actual_adapter="$adapter"
    
    # Check if the input looks like a MAC address
    if validate_mac_address "$adapter"; then
        log "MAC address detected: $adapter"
        
        # Find the interface with this MAC address
        actual_adapter=$(find_interface_by_mac "$adapter")
        
        if [ -z "$actual_adapter" ]; then
            error_exit "No network adapter found with MAC address '$adapter'. Please check available adapters and their MAC addresses with 'ip link show'"
        fi
        
        log "Found interface '$actual_adapter' with MAC address '$adapter'"
        
        # Update the global variable to use the found interface name
        RESOLVED_ADAPTER="$actual_adapter"
    else
        # Input is not a MAC address, treat as interface name
    ...
    log "Network adapter '$RESOLVED_ADAPTER' validated successfully"
}
...
show_help() {
    cat << EOF
Usage: $0 <network_adapter|mac_address> <command|ip_address>
...
Arguments:
    network_adapter  The network interface to configure (e.g., eth0, enp0s3)
                     OR MAC address with colon or dash delimiters 
                     (e.g., 00:11:22:33:44:55 or 00-11-22-33-44-55)
...
Examples:
    ...
    $0 00:11:22:33:44:55 192.168.1.100  Configure interface with MAC address
    $0 00-11-22-33-44-55 192.168.1.100  Configure interface with MAC address (dash format)
    $0 eth0 clean                      Clean up configuration on eth0
    $0 00:11:22:33:44:55 status        Show status of services for interface with MAC
...
main() {
    ...
    if [ -z "$1" ]; then
        error_exit "Please provide a network adapter name or MAC address and command. Use -h for help."
    fi
    
    local adapter=$1
    
    # Validate adapter (this sets RESOLVED_ADAPTER global variable)
    ...
    case "$2" in
        "clean")
            cleanup_services "$RESOLVED_ADAPTER"
            log "System restored to original state"
            exit 0
            ;;
        "status")
            check_status "$RESOLVED_ADAPTER"
            exit 0
            ;;
        *)
            ...
            configure_static_ip "$RESOLVED_ADAPTER" "$static_ip"
            setup_tftp "$RESOLVED_ADAPTER" "$static_ip"
            test_tftp "$static_ip"
            setup_nfs "$RESOLVED_ADAPTER" "$static_ip"
            test_nfs "$static_ip"
            ...
            log "Static IP configured: $static_ip on adapter $RESOLVED_ADAPTER"
            log ""
